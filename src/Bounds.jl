...
function deviation_box(b::ParameterBounds, theta0::AbstractVector, px::Integer, py::Integer)
    lims = map((px, py)) do idx
        if b.periodic[idx]
            ...
            (lo < 0.0 && hi > 0.0) ||
                error("Base point component $idx (= $(theta0[idx])) is not strictly inside " *
...
infinite.
"""
function ray_box_crossing(cphi::Real, sphi::Real, lox::Real, hix::Real, loy::Real, hiy::Real)
...
function clamp_interior(theta::AbstractVector, b::ParameterBounds; margin::Real = 1e-8)
    ...
    for i in eachindex(out)
        ...
        m = isfinite(lo) && isfinite(hi) ? margin * (hi - lo) : margin * max(1.0, abs(out[i]))
